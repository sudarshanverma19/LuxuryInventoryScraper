"""
Quick test — only scrapes 3 products to verify Acne Studios scraper works.
Run from backend folder: python test_acne.py
"""
import asyncio
...
async def test():
    print("Testing Acne Studios scraper (quick — 3 products only)...")
    s = AcneScraper()
    ...
    if len(products) < 3:
        ...
        print('  console.log("PRICE:", document.querySelector(".price")?.innerText)')
        print('  document.querySelectorAll("button.variations__grid-item").forEach(el =>')
